...
***There are `r nrow(lq_ak)` La Quinta's locations in Alaska.***
...
***There are 6 pairings between all Denny's and all La Quinta's location in Alaska.***
...
***There are `r nrow(dn_lq_ak)` observation in the joined dataset. The names of the variables are `r names(dn_lq_ak)`.***
...
ggplot(dn_lq_ak_mindist, aes(x = closest)) +
  geom_histogram() +
  labs(title = "Minimum distance between Denny's and La Quinta's", subtitle = "Alaska", x = "Distance", y = "Density")

dn_lq_ak_mindist %>%
  summarise(mean_dist = mean(closest))
```

***The average distance between Denny's and La Quinta's locations is 4.41 km. From the visualization, we can see that 2 of the 3 location distances are above 5 km. The min is around 2 km and the max is around 6 km.***
...
ggplot(dn_lq_nc_mindist, aes(x = closest)) +
  geom_histogram() +
  labs(title = "Minimum distance between Denny's and La Quinta's", subtitle = "North Carolina", x = "Distance", y = "Density")

dn_lq_nc_mindist %>%
  summarise(mean_dist = mean(closest))
```

***The average distance between Denny's and La Quinta's locations is 65.4 km. From the visualization, we can see that the data has a clear mode around 40 km, and a possible secondary mode near 115 km. The data is skewed to the high end. The min is around 0 km and the max is around 175.***
...
ggplot(dn_lq_tx_mindist, aes(x = closest)) +
  geom_histogram() +
  labs(title = "Minimum distance between Denny's and La Quinta's", subtitle = "Texas", x = "Distance", y = "Density")

dn_lq_tx_mindist %>%
  summarise(mean_dist = mean(closest))
```

***The average distance between Denny's and La Quinta's locations is 5.79 km. From the visualization, we can see that the data has a clear mode around 0 km (less than 1). The data, however, is skewed to the high end. The min is around 0 km and the max is around 60 km.***
...
ggplot(dn_lq_me_mindist, aes(x = closest)) +
  geom_histogram() +
  labs(title = "Minimum distance between Denny's and La Quinta's", subtitle = "Maine", x = "Distance", y = "Density")

dn_lq_me_mindist %>%
  summarise(mean_dist = mean(closest))
```

***The average distance between Denny's and La Quinta's locations is 86.57 km. From the visualization, we can see that the 2 of the 7 pairings has a distance of around 180 km. Most, however, have a distance of less than 100 km. The min is around 0 and the max is around 180.***

12. Among the states you examined, where is Mitch Hedberg's joke most likely to hold true?
    Explain your reasoning.

***Among the states I examined, Mitch Hedberg's joke is most likely to hold true in Alaska. For this state the mean was the lowest (4.91 km) and the max was substantially the lowest. The mean is also low in Texas (5.79 km), however the max is quite high (around 60 km).***
